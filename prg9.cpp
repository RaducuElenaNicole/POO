// Relatia HAS-A -> compunere 1:M cu vector dinamic de pointer la obiecte
// vector de pointer (**)
...
public:
	...
	Grupa() {
		this->numeGrupa = "N/A";
		this->nrStud = 1;
		this->vectStud = new Student * [this->nrStud] {new Student()};
	}
	...
	Grupa(string numeGrupa, int nrStud, Student** vectStud) {
		...
		for (int i = 0; i < this->nrStud; i++) {
			this->vectStud[i] = new Student(*vectStud[i]);
		}
	}
	...
	Grupa(const Grupa& grp) {
		...
		for (int i = 0; i < grp.nrStud; i++) {
			this->vectStud[i] = new Student(*grp.vectStud[i]);
		}
	}
	...
	friend ostream& operator<<(ostream& out, const Grupa& grp) {
		out << "Grupa --- NUME: " << grp.numeGrupa << " | NR STUD: " << grp.nrStud << " | STUDENTI: " << endl;
		for (int i = 0; i < grp.nrStud; i++) {
...
void main() {
	...
	Student** studenti1;
	int nrStudenti1 = 3;
	studenti1 = new Student * [nrStudenti1] {&s1, &s2, &s3};
	Grupa g1("A_101", nrStudenti1, studenti1);
	cout << "g1 -> " << g1;

	Student** studenti2;
	int nrStudenti2 = 2;
	studenti2 = new Student * [nrStudenti2] {&s5, &s4};
	Grupa g2("B_102", nrStudenti2, studenti2);
	cout << "g2 -> " << g2;

	Student** studenti3;
	int nrStudenti3 = 5;
	studenti3 = new Student * [nrStudenti3] {&s1, &s2, &s3, &s5, &s4};
	Grupa g3("C_103", nrStudenti3, studenti3);
	cout << "g3 -> " << g3;
